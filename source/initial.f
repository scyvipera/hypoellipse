c initial.for    []
      subroutine initial(intype, eqname)
c get filenames, open files and write greeting for hypoellipse
      include 'params.inc' 
      character*(*) eqname   
      character*256 root, arcname
      integer punt
      common /punt/ punt
      common /dhip/ inpt,isa,ilis,inmain,injump
      logical eoff
      common /dph/ noswt, eoff
      character*4 ipro, ichec, evtype*1, evstat*1
      common /dbhip/ ipro, ichec, evtype, evstat, root
      common /hf/ cp(72),sp(72)
      real*8 time1, time2
      common /hop/ time1,time2,nopu,notim
      common /it/ vpvs(lmax2),vpvsm(mmax+3),nttab
      common /lm/ mapend
      common /pmost/ nr,nrp,lbastm,tp(npa),ksmp(npa),kdx(npa)
      integer summary_unit, arch_unit
      parameter (summary_unit = 4)
      parameter (arch_unit    = 11)
      character*1 print_type, summary_type, arch_type
      parameter (print_type   = 'o')
      parameter (summary_type = 's')
      parameter (arch_type    = 'a')
      character openstat*7

      openstat = 'unknown'

c     write(0,'(" initial mode=",i2," eqname=",a)')intype,eqname

      if(intype .eq. 1) then
c open the files for hypoellipse (returns root name)
        call opfls(inpt, inmain, root, nttab)
c       write(0,'(" root name returned by opfls = ",a)') root
c ignore everything in output root after last / (ignore previous eq name)
        do 35 i = lentru(root), 1, -1
          if(root(i:i) .eq. '/') goto 40
35      continue
c no / found?
        i = 2
        root = './'
40      length_root = i

        call timit(0)
c get current date
        call jdate(irmo, irdy, iryr, ihr, imn, isec)
c do not print output conversion errors
        call erset (63,.true.,.false.,.false.,.false.)
        write(punt,20) nsn, npa, iryr, irmo, irdy, ihr, imn
20      format (
     *' *** Hypoellipse: Unix/Xpick/Y2K version 3.9 11/1/2001  ***',
     */, '  Configured for up to ', i4, ' stations in station list',
     */, '  and up to ', i4, ' records per earthquake.',
     */, '  run on ', 2(i2.2, '/'), i2.2, ' at ', i2.2, ':', i2.2)
c calculate tables to be used in focal mechanism plots
        do 30 i=1,72
c         pi=i*.0349066*2.5
          pi = i*0.0872665
          cp(i)=cos(pi)
          sp(i)=sin(pi)
30      continue
        ichec = 't s '
        mapend=0
        nr = 0
        time1 = 0.0

      else

c --- earthquake input file
        call openfl (inpt,eqname,'old','zero','readonly','noprint',0)

        rewind (inpt)
  
        eoff = .false.
        len = lentru(eqname)
        arcname = eqname
  
        if (arcname(len:len).ne.'p') then
          len = len + 1
        endif
        arcname(len:len) = 'a'
  
c       write (0,'(" trying to open (unit ",i2,") ",a)')
c    &    arch_unit, arcname

        call openfl (arch_unit,arcname,openstat,'null','none',
     &               'noprint',0)
c       write (0,'("opened new files, eqname=",a," arcname=",a)')
c    &    eqname, arcname

      endif

      return
      end
c end initial
